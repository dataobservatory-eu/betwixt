...
delini_dual_wide <- create_candidate_dataset(
  ...
  input_predicate_range = add_candidate_range(
    "depicts",
    "documents",
    "is evidence for",
    "Other…"
...
test_that("Delini wide and dual-wide fixtures are consistent", {
  expect_false("input_predicate" %in% names(delini))
  expect_true("input_predicate" %in% names(delini_dual_wide))
  expect_equal(delini_dual_wide$input_predicate, rep("depicts", 5L))
