...
One useful way of representing candidate knowledge in `Betwixt` is as a scoped claim:
...
The reference implementation presents bounded candidate assertions for review while preserving descriptive, evidential, and contextual information around them.

Review outcomes are intentionally simple. A candidate assertion may be accepted or modified, rejected, or deferred when further evidence or review is required.

- `accepted`: the candidate or modified assertion is accepted for the current review purpose;

- `rejected`: the candidate assertion should not be accepted;

- `deferred`: the assertion requires additional evidence, another review procedure, or another reviewer.
...
Betwixt treats review as a transformation of candidate semantic state. Candidate data are projected into review interfaces, where human decisions can stabilise, modify, reject, or defer the assertions presented for review. The review activity and its evolving state are recorded separately from the candidate assertions themselves.

The reference implementation renders candidate datasets into lightweight standalone HTML review documents that can be used directly in a web browser. These documents are intended to minimise cognitive burden by presenting only the semantic components that require human judgement. In the simplest workflow, the review target is the value component of a claim. More advanced implementations may allow review of the subject, predicate, or any combination of semantic components.
...
- corroborate: the reviewed claim is sufficiently stable for the intended purpose;
...
Conceptually, an atomic scoped claim may be represented as:
...
`Betwixt` provides a lightweight model for transforming candidate semantic assertions into bounded, human-reviewable objects. By separating candidate knowledge, review projections, human decisions, and review provenance, it provides a review layer that can be integrated into a wide range of interoperability, harmonisation, and knowledge management workflows.
